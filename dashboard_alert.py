...
class AlertApp(QWidget):
    def __init__(self, conf, layout, topic, side):
        ...
        self.layout = layout
        if side == 'left':
            self.setGeometry(self.layout['left_gripper_alarm']['x'], self.layout['left_gripper_alarm']['y'], 200, 400)
            self.font_size = self.layout['left_gripper_alarm']['font_size']
            self.setWindowTitle("Left Gripper Alarm")
            self.label = QLabel(self)
            self.label.setGeometry(self.layout['left_gripper_alarm']['label_x'], self.layout['left_gripper_alarm']['label_y'], 300, 50)

        else:
            self.setGeometry(self.layout['right_gripper_alarm']['x'], self.layout['right_gripper_alarm']['y'], 200, 400)
            self.font_size = self.layout['right_gripper_alarm']['font_size']
            self.setWindowTitle("Right Gripper Alarm")
            self.label = QLabel(self)
            self.label.setGeometry(self.layout['right_gripper_alarm']['label_x'], self.layout['right_gripper_alarm']['label_y'], 300, 50)
    ...
    def callback(self, msg):
        ...
        if force_module > (self.conf['limit_force']- 5.0 ) or torque_module > (self.conf['limit_torque'] - 3.0):
